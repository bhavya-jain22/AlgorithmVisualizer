...
public class PacketTracerGUI extends JFrame {
    ...
    public PacketTracerGUI(NetworkGraph graph, SimulationController controller) {
        ...
        JLabel titleLabel = new JLabel("Routing from Router " + startRouter.id + " to Router " + endRouter.id);
        titleLabel.setFont(new Font("Segoe UI", Font.BOLD, 18));
        titleLabel.setForeground(TEXT_LIGHT);
        headerPanel.add(titleLabel);
        
        algoSelector = new JComboBox<>(new String[]{"Show A*", "Show Dijkstra", "Show Bellman-Ford"});
        algoSelector.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        algoSelector.addActionListener(e -> updateDisplayedPath());
        headerPanel.add(algoSelector);
        ...
        headerPanel.add(runBtn);
        
        raceBtn = new JButton("🏆 Race All");
        raceBtn.setBackground(new Color(155, 89, 182));
        raceBtn.setForeground(Color.WHITE);
        raceBtn.setFocusPainted(false);
        raceBtn.addActionListener(e -> startRace());
        headerPanel.add(raceBtn);
        
        benchmarkBtn = new JButton("🔥 Harsh Benchmark");
        benchmarkBtn.setBackground(new Color(231, 76, 60)); // Red
        ...
        JTextArea hintText = new JTextArea("Hint: Click any cable on the map to BREAK it. Watch the algorithms dynamically route around the outage!");
        hintText.setWrapStyleWord(true);
        hintText.setLineWrap(true);
        hintText.setEditable(false);
        hintText.setBackground(PANEL_DARK);
        hintText.setFont(new Font("Segoe UI", Font.ITALIC, 13));
    ...
    private void startRace() {
        ...
        // A* = Green, Dijkstra = Blue, Bellman = Purple
        // Using offsets to prevent them from overlapping if they pick the exact same path
        if (aStar != null && aStar.path != null) 
            networkPanel.activePackets.add(new NetworkPanel.Packet("A*", aStar.path, new Color(46, 204, 113), -15));
            
        if (dijkstra != null && dijkstra.path != null) 
            networkPanel.activePackets.add(new NetworkPanel.Packet("Dijkstra", dijkstra.path, new Color(52, 152, 219), 0));
            
        if (bellman != null && bellman.path != null) 
            networkPanel.activePackets.add(new NetworkPanel.Packet("Bellman", bellman.path, new Color(155, 89, 182), 15));
